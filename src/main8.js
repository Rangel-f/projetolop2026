...
let keymap = []; //armazenando apenas as teclas que estão sendo recebidas no momento
let posemap = []; //armazenando apenas as poses que estão sendo recebidas no momento
//
...
//const canvas_tf = document.querySelector("#meu_canvas");

let flag_captura = 0; //flag de controle da ativação/desativação do controle do carro pelas poses
let flag_c = 1; //flag para capturar o pressionar e soltar da tecla C;
...
async function detectarPoses() {
    ...
    if(flag_captura)
    {
        ...
        //console.log("Entrou aqui na detecção!");
        ...
        if (poses.length > 0) {
            ...
            //console.log(keypoints);
                    ...
                    const cotoveloDireito = keypoints[8];
                    const cotoveloEsquerdo = keypoints[7];

                    if(pulsoDireito.score > 0.3 && pulsoEsquerdo.score > 0.3) 
                        {
                            //console.log(pulsoDireito.y);
                            //console.log(pulsoEsquerdo.y);
                            if(pulsoDireito.y < ombroDireito.y && pulsoEsquerdo.y < ombroEsquerdo.y)
                                {
                                    console.log("Baixa as mão!");
                                    if(posemap.indexOf("Mãos_Levantadas") == -1) posemap.push("Mãos_Levantadas");
                                }
                            else if(posemap.indexOf('Mãos_Levantadas') != -1)
                                {
                                    posemap.splice(posemap.indexOf('Mãos_Levantadas'), 1);
                                } 


                            if(pulsoDireito.x > ombroDireito.x && pulsoEsquerdo.y < ombroEsquerdo.y && pulsoDireito.y > ombroDireito.y && pulsoDireito.y < cotoveloDireito.y)
                                {
                                    if(posemap.indexOf("Câmera_girar_esquerda") == -1) 
                                        {
                                            posemap.push("Câmera_girar_esquerda");
                                            //console.log(posemap);
                                        }
                                }
                            else if(posemap.indexOf('Câmera_girar_esquerda') != -1)
                                {
                                    posemap.splice(posemap.indexOf('Câmera_girar_esquerda'), 1);
                                } 


                            if(pulsoDireito.x < ombroDireito.x && pulsoEsquerdo.y < ombroEsquerdo.y && pulsoDireito.y > ombroDireito.y && pulsoDireito.y < cotoveloDireito.y)
                                {
                                    if(posemap.indexOf("Câmera_girar_direita") == -1) 
                                        {
                                            posemap.push("Câmera_girar_direita");
                                            //console.log(posemap);
                                        }
                                }
                            else if(posemap.indexOf('Câmera_girar_direita') != -1)
                                {
                                    posemap.splice(posemap.indexOf('Câmera_girar_direita'), 1);
                                } 
...
create_box(0.5, 10, 0.5, materials_standart.preto, 4, 0, -100, 'parada');
    ...
    const parede_geometry = new THREE.BoxGeometry(1000, 8, 5);
    const parede_mesh = new THREE.Mesh(parede_geometry, materials_standart.cinza); //parede
    //scene2.add(parede);
    parede_mesh.position.set(-502, 1, 0);
    parede_mesh.rotation.y = 3.1415/2;

    const parede_box3 = new THREE.Box3().setFromObject(parede_mesh);
    //const parede_boxHelper = new THREE.Box3Helper(parede_box3, 0x0000ff);
    //parede_mesh.add(parede_boxHelper);

    //scene2.add(parede_boxHelper);
    scene2.add(parede_mesh);
    //console.log("Parede: " + parede_boxHelper);
...
function animate(time)
{
    ...
    */
    ...
    Box3Helper_carcaca.setFromObject(carcaca_clone);
            let x_c = carcaca.position.x;
            let z_c = carcaca.position.z;
    //
    ...
    else // ==> cena 2
    {
        ...
        const line_ponto_medio = new THREE.Vector3((line_x_position.x + line_x_position_z/2), 0, (line_x_position.z + line_x_position_x/2));

        //esf_blue.lookAt(parada_position);

// ======== Colisão da carcaça com as moedas ========
...
// ======== Ativação/Desativação do controle por captura de poses ========
        ...
        if(keymap.indexOf('C') != -1)
        {
            ...
            console.log(flag_captura);
            if(flag_captura) document.getElementById('estado_da_flag').innerHTML = 'ON';
            else document.getElementById('estado_da_flag').innerHTML = 'OFF';
        }
        else flag_c = 1;
...
// ================== CONTROLE DA CÂMERA ==================
        ...
        if(keymap.indexOf('ARROWRIGHT') != -1 || posemap.indexOf('Câmera_girar_esquerda') != -1)
        {
            console.log("ARROWDOWN");
            central_orbit.rotation.y += 0.02;
        }
        if(keymap.indexOf('ARROWLEFT') != -1 || posemap.indexOf('Câmera_girar_direita') != -1)
        {
...
// ================== CONTROLE DO CARRO ==================
        ...
        if(keymap == 0 && posemap == 0 && !flag_inatividade)
            {
                if(inative == 0)
                {
                    inative = time;
        ...
        else if((keymap != 0 || posemap != 0) && !flag_inatividade)
            {
                flag_inatividade = 0;  
                inative = 0;
            } 
        ...
        if(!flag_inatividade)
        {
            ...
            if(keymap.indexOf('W') != -1 || posemap.indexOf('Mãos_Levantadas') != -1)
                {
        ...
        // ======== Colisão da carcaça com a parede ========

        if(Box3Helper_carcaca.intersectsBox(parede_box3))
        {
            carcaca.position.set(x_c, carcaca.position.y, z_c);
            console.log("colisão!");
            if(parede_box3.containsPoint(line_ponto_medio))
                {
                    increase_velocidade = 0;
                } 
            //flag_colisao = 1;
        }
        //else flag_colisao = 0;
